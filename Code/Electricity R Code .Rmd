...
merged <- merge(x = merged, y = blocks_df, by.x = "id", by.y = "GEOID10")  
...
```
```{r}
df$Per.Capita.Income..2014 <- as.data.frame(df$Per.Capita.Income..2014)
```


```{r}
model <- lm(Avg..Monthly.Household.Electricity.Use..2014 ~ pm_hi + Per.Capita.Income..2014, data = df)
summary(model)
```
